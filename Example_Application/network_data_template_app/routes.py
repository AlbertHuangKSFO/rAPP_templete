...
async def topology():
    ...
    try:
        ...
        logger.error(f"Topology Timeout: {str(e)}")
        return JSONResponse({"Error": "Topology service timed out."}, 503)
    except RequestError as e:
        logger.error(f"Topology Request Error: {str(e)}")
        return JSONResponse({"Error": "Failed to connect to Topology service."}, 500)
    except HTTPStatusError as e:
        logger.error(f"{str(e.response.status_code)} HTTP Status Error from Topology service: {str(e)}")
        return JSONResponse({"Error": "Topology endpoint returned an error: ", "Response": str(e)}, 500)
...
async def network_configuration(attribute: str = "operationalState"):
    ...
    try:
        ...
        logger.error(f"Network Configuration Timeout: {str(e)}")
        return JSONResponse({"Error": "Network Configuration service timed out."}, 503)
    except RequestError as e:
        logger.error(f"Network Configuration Request Error - {str(e)}")
        return JSONResponse({"Error": "Failed to connect to Network Configuration service."}, 500)
    except HTTPStatusError as e:
        logger.error(f"{str(e.response.status_code)} HTTP Status Error from Network Configuration service - {str(e)}")
        return JSONResponse({"Error": "Network Configuration endpoint returned an error: ", "Response": str(e)}, 500)
