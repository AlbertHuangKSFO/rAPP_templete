...
@pytest.fixture
def data_management_api(mock_apis, config):
    ...
    with open(
        ...
        mock_apis.get(data_management_endpoint) % Response(
...
@pytest.fixture
def data_management_api_500_response(mock_apis, config):
    data_management_endpoint = (
        config.get("iam_base_url")
        + "/dmm-data-collection-controller/data-access/v2/"
        + config.get("iam_client_id")
    ...
    mock_apis.get(data_management_endpoint) % Response(status_code=500)
...
def topology_api(mock_apis, config):
    ...
    mock_apis.get(
        f"{topology_endpoint}/domains/RAN/entity-types/NRCellDU/entities?targetFilter=/sourceIds&limit=10",
        headers__contains=expected_headers,
    ) % Response(status_code=200, json=response_json)
    mock_apis.get(topology_matcher) % Response(status_code=403, text="Forbidden")
...
def topology_not_ok_api(mock_apis, config):
    topology_endpoint = (
        config.get("iam_base_url")
        + "/topology-inventory/v1alpha11/domains/RAN/entity-types/NRCellDU/entities?targetFilter=/sourceIds&limit=10"
    )
...
def network_configuration_not_ok_api(mock_apis, config):
    network_configuration_pattern = (
        config.get("iam_base_url")
        + r"\/ncmp\/v1\/ch\/.*\/data\/ds\/ncmp-datastore:.*\?resourceIdentifier=.*"
    )
    network_configuration_matcher = re.compile(network_configuration_pattern)
    mock_apis.get(network_configuration_matcher) % Response(status_code=500)
    yield mock_apis
...
def kafka_consumer_with_valid_messages():
    ...
    with open("tests/kafka_valid_msg_headers.json", "rb") as file:
        headers = json.load(file)
    mock_header = [(k.encode("utf-8"), v.encode("utf-8")) for k, v in headers]
    ...
    with open("tests/kafka_invalid_msg_headers.json", "rb") as file:
        headers = json.load(file)
    mock_header = [(k.encode("utf-8"), v.encode("utf-8")) for k, v in headers]
    ...
    with open("tests/kafka_valid_msg_headers_with_diff_nodefdn.json", "rb") as file:
        headers = json.load(file)
    mock_header = [(k.encode("utf-8"), v.encode("utf-8")) for k, v in headers]

    msg5.headers.return_value = mock_header
    msg5.error.return_value = None

    msg6 = MagicMock()

    with open("tests/kafka_valid_msg_headers_with_no_schema_id.json", "rb") as file:
        headers = json.load(file)
    mock_header = [(k.encode("utf-8"), v.encode("utf-8")) for k, v in headers]

    msg6.headers.return_value = mock_header
    msg6.error.return_value = None
...
@pytest.fixture
def kafka_consumer_with_no_messages():
    """Kafka consumer that consumes no messages."""
    consumer = MagicMock()
    consumer.consume.return_value = []  # Simulate `consume()` returning empty messages
    yield consumer
...
@pytest_asyncio.fixture
async def message_bus_consumer_consumes_no_messages(
    sync_oauth_client,
    async_oauth_client,
    kafka_consumer_with_no_messages,
    data_management_url,
    get_topology_get_nr_cell_dus_response,
):
    """Async fixture for MessageBusConsumer that consumes no messages."""
    with patch(
        "network_data_template_app.message_bus_consumer.get_nr_cell_dus",
        new_callable=AsyncMock,
    ) as mock_get_nr_cell_dus:
        mock_get_nr_cell_dus.return_value = get_topology_get_nr_cell_dus_response
        consumer_instance = MessageBusConsumer(
            sync_oauth_client, async_oauth_client, kafka_consumer_with_no_messages
        )
        await consumer_instance._fetch_prefixed_fdns()
        yield consumer_instance
...
def log_control_file():
    with open("./tests/logcontrol.json", "r", encoding="utf-8") as f:
        os.environ["LOG_CTRL_FILE"] = json.dumps(f.read())
    yield
...
def populate_environment_variables():
    ...
    os.environ["CONSUMER_MESSAGE_BATCH_SIZE"] = "10"
